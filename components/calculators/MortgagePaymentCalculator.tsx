...
function compute(s: Scenario) {
  const baseLoan = Math.max(0, s.price - s.down);
  const cmhc = cmhcPremium(s.price, s.down);
  const principal = baseLoan + cmhc.premium;
  ...
  return { principal, cmhc, plan, freqLabel };
}
...
export default function MortgagePaymentCalculator() {
    ...
    .map(
      (r, i) =>
        `${multi ? `Scenario ${String.fromCharCode(65 + i)} — ` : ''}${r.freqLabel} payment: ${fmtDollars(
          r.plan.payment,
          2
        )}, total interest: ${fmtDollars(r.plan.totalInterest)}`
    )
    .join('\n');
  ...
  const rows = [
    ...
    {
      label: 'Mortgage Insurance (CMHC)',
      values: results.map((r) => (r.cmhc.required ? fmtDollars(r.cmhc.premium) : 'N/A')),
    },
    { label: 'Closing Costs', values: scenarios.map((s) => fmtDollars(s.closing)) },
  ];

  const ScenarioInputs = (s: Scenario, i: number) => (
    <div
      key={i}
      className={multi ? 'rounded-xl border border-border p-5' : ''}
    >
      {multi && (
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        <Field label="Home Price" guided={guided} helper="The purchase price of the home.">
          <CurrencyInput value={s.price} onChange={(n) => update(i, { price: n })} />
        </Field>
        <Field label="Down Payment" guided={guided} helper="Cash toward the purchase. Under 20% adds CMHC insurance.">
          <CurrencyInput value={s.down} onChange={(n) => update(i, { down: n })} />
        </Field>
        <Field label="Amortization" guided={guided} helper="Years to pay off the mortgage.">
        ...
        <Field label="Closing Costs" guided={guided} helper="Legal, title, and other one-time costs.">
          <CurrencyInput value={s.closing} onChange={(n) => update(i, { closing: n })} />
        </Field>
  ...
  const right = (
    <ResultCard title="Your mortgage payment" summary={summary}>
